...
def run_episode(env: RestrictedRPSEnv, policy: str = "random", verbose: bool = True) -> dict:
    ...
    policy:
        ...
        "legal"     — pick a random action from non-exhausted moves only
    """
    obs, _ = env.reset()
    total_reward = 0.0
    steps = 0
    ...
    while True:
        if policy == "legal":
            legal = env._agent.available_moves()
            if not legal:
                action = env.action_space.sample()
            else:
                action = legal[np.random.randint(len(legal))].value
        else:
            action = env.action_space.sample()
        ...
        if verbose:
            move_name = ["ROCK", "PAPER", "SCISSORS"][action]
            print(f"  Step {steps:>3} | action={move_name:<8} reward={reward:+.1f} "
                  f"total={total_reward:+.1f} | obs={obs}")
...
if __name__ == "__main__":
    ...
    env = RestrictedRPSEnv(n_opponents=3, lives=3, budget=4)
    check_spaces(env)
    env.close()

    # ── 3. aggregate stats over 200 episodes ─────────────────────────────────
    run_suite(n_episodes=200, policy="legal", n_opponents=3, lives=3, budget=4)